...
import Student from "../assets/gifs/Student.gif"
...
export const FilteredResouces = () => {
  ...
  return (
    <div className='w-full h-screen flex justify-between bg-[#000000] flex-col'>
      <div className='w-full flex justify-center flex-col text-white mt-4'>
        <h1 className='w-full text-center text-2xl md:text-4xl'>{Department}</h1>
        <span className='w-full text-center text-lg md:text-xl mt-2'>{level}</span>
      </div>
      <div className='w-full flex justify-evenly flex-wrap gap-y-4 h-[50%] items-center'>
        {
          Resources.map((resource, index)=>(
            <div key={index} className='group relative w-[150px] h-[150px] md:w-[30%] md:min-w-[180px] md:max-w-[200px] md:h-[200px] bg-[#392B1A] border-white border rounded-md cursor-pointer active:scale-90 duration-500 transition-all ease-in-out'>
      ...
      <div className='w-full flex justify-center h-[20%] md:h-[30%] items-center'>
