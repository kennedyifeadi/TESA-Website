...
export const SocialsCard = ({text, icon, hoverIcon, link}) => {
  ...
  return (
    ...
    >
      <span className={`${isHovered ? "translate-y-[-20px] text-white" : "translate-y-0"} text-black text-[10px] md:text-2xl font-medium duration-500 ease-in-out transition-all`} style={{fontFamily:'"Poppins", sans-serif'}}> <a href={link}>{text}</a></span>
      {
        isHovered ? (
